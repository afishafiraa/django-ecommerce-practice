...
class HomeListBathBody(generic.ListView):
    ...
    queryset =  ProdukItem.objects.filter(kategori__contains='BB')
    paginate_by = 4

class HomeListSkincare(generic.ListView, django_filters.FilterSet):
    template_name = 'skincare.html'
    queryset = ProdukItem.objects.filter(kategori__contains='S')
    paginate_by = 4

class HomeListOthers(generic.ListView, django_filters.FilterSet):
    template_name = 'others.html'
    queryset = ProdukItem.objects.filter(kategori__contains='OW')
    paginate_by = 4

class ContactView(generic.ListView, django_filters.FilterSet):
...
def search_feature(request):
    ...
    if request.method == 'POST':
        ...
        posts = ProdukItem.objects.filter(nama_produk__icontains=search_query)
        return render(request, 'searchpost.html', {'query':search_query, 'posts':posts})
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            EmailMessage(
               ...
               ['cs@skinshop.com'], # Send to (your admin email)
               [],
